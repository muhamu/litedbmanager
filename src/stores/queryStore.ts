...
export interface TableTab {
  ...
  tableName: string;
  objType: "table" | "view";
  defaultSection?: "properties" | "data" | "ddl";
}

export type Tab = SqlTab | TableTab;
export type QueryTab = SqlTab; // backwards-compat alias
...
export const useQueryStore = create<QueryState>((set, get) => ({
  ...
  newTab: (sql) => {
    const id = uuid();
    const tab: SqlTab = {
      id, kind: "sql", name: nextName(), sql: sql ?? "",
      result: null, error: null, running: false, modified: false,
    };
    set((s) => ({ tabs: [...s.tabs, tab], activeTabId: id }));
  },

  openTableTab: (database, tableName, objType, section) => {
    const { tabs } = get();
    const existing = tabs.find(
      (t) => t.kind === "table" && t.database === database && t.tableName === tableName,
    );
    if (existing) { set({ activeTabId: existing.id }); return; }
    const id = uuid();
    const tab: TableTab = { id, kind: "table", name: tableName, database, tableName, objType, defaultSection: section };
    set((s) => ({ tabs: [...s.tabs, tab], activeTabId: id }));
  },
  ...
  closeTab: (id) => {
    set((s) => {
      ...
      if (s.activeTabId === id) {
        newActive = newTabs.length === 0 ? null : idx > 0 ? newTabs[idx - 1]!.id : newTabs[0]?.id ?? null;
      }
  ...
  updateSql: (id, sql) =>
    set((s) => ({ tabs: s.tabs.map((t) => (t.id === id ? { ...t, sql, modified: true } : t)) })),

  setResult: (id, result) =>
    set((s) => ({ tabs: s.tabs.map((t) => (t.id === id ? { ...t, result, error: null, running: false } : t)) })),

  setError: (id, error) =>
    set((s) => ({ tabs: s.tabs.map((t) => (t.id === id ? { ...t, error, result: null, running: false } : t)) })),

  setRunning: (id, running) =>
    set((s) => ({ tabs: s.tabs.map((t) => (t.id === id ? { ...t, running } : t)) })),

  renameTab: (id, name) =>
    set((s) => ({ tabs: s.tabs.map((t) => (t.id === id ? { ...t, name, modified: false } : t)) })),
    ...
    set((s) => {
      ...
      const list =
        existing >= 0
          ...
          : [...s.savedQueries, { name, sql: tab.sql }];
      return { savedQueries: list };
    });
  },

  loadQuery: (sql) => get().newTab(sql),
}));
